"""Simple vision-language model training loop with synthetic data, logging, and checkpointing."""
...
def _default_save_dir(model_id: str) -> Path:
    ...
    return Path("runs") / "vlm" / safe_name


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a vision-language model on synthetic data.")
    parser.add_argument("--model-id", type=str, default="qwen3-vl-smoke")
...
def main() -> None:
    ...
    _, last_metrics = vlm_trainer.run_training(
        args.model_id,
        train_cfg,
        save_dir=save_dir,
