...
def _process_conversation(
    ...
    *,
    ...
    def _make_example(cur_msgs, cur_len, cur_vt, cur_vp, cur_ni):
        ...
        if not cur_msgs:
            return None
        # Skip chunks whose loss mask would be all zeros (no supervised assistant
        # tokens; assistant turns marked "loss": false count as context).
        if not any(message_is_supervised(m) for m in cur_msgs):
            return None
