...
def _validate_cached_measurement(measurement: Any, field: str) -> None:
    ...
    for name in ("terminal_length_delta", "terminal_supervised_tokens_delta"):
        if type(measurement[name]) is not int:
            raise ValueError(f"{field}.{name} must be an integer")
    terminal_length = measurement["length"] + measurement["terminal_length_delta"]
    terminal_supervised = (
        measurement["supervised_tokens"] + measurement["terminal_supervised_tokens_delta"]
    )
    if terminal_length < 0 or not 0 <= terminal_supervised <= terminal_length:
        raise ValueError(f"{field} has invalid terminal measurements")
    ...
    for message in messages:
        ...
        for block in content:
            if not isinstance(block, dict) or block.get("type") not in (
                ...
                continue
            value = block.get("image", block.get("image_url"))
