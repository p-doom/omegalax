...
def _is_vision_path(path) -> bool:
    """True if a state-tree path passes through a ``vision`` attribute — i.e. the
    leaf belongs to the vision tower (``Qwen3VL.vision`` / ``Qwen3_5...vision``).
    """
...
def _vision_label_fn(params):
    """Label tree for ``optax.multi_transform``: tag each leaf ``"vision"`` if it
    belongs to the vision tower, else ``"text"``. The returned tree matches the
    structure of the pure param pytree ``MixedPrecisionOptimizer`` feeds to the
    transform.
    """
...
def build_optimizer(
    ...
    *,
    ...
    if vision_lr_schedule_fn is not None:
        # Route vision-tower leaves through a dedicated AdamW with its own LR
        # schedule; every other trainable leaf uses the main schedule. Note the
        # clip_by_global_norm above and any MultiSteps below still wrap BOTH
        # groups together -- one global grad-norm, one accumulation counter.
