...
on forward and backward, up to fp reduction-order roundoff. (The dense einsum lives
here as the slow reference used to validate the fast kernel.) The ``grouped_moe_ep``
path is checked the same way on faked multi-CPU devices (EP=2, EP=4); ``ragged_all_to_all``
is emulated via all_gather on CPU (see moe_grouped).
