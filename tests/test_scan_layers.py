...
class Qwen3VLScanEquivalenceTest(absltest.TestCase):
    ...
    # NOTE on tolerance: Qwen3-VL text attention hard-casts q/k/v to bfloat16
    # before ``dot_product_attention`` (model.py TextAttention.__call__), even in
    # an fp32 config. That bf16 roundtrip has a lossy VJP, and the scan reverse
    # pass vs the unrolled reverse pass accumulate those lossy cotangents in a
    # slightly different order, giving ~1e-3 grad differences that GROW with
    # depth and do NOT shrink at fp64 (verified). The forward hidden states are
    # bit-identical (0.0). Removing the bf16 cast drops the grad diff to ~4e-7,
    # so this is inherent model behavior, not a scan-correctness issue. We use a
    # depth-appropriate grad tolerance here and assert exact-ish loss.
    def _run_case(
        self,
        cfg,
        loss_atol=1e-4,
    ...
    def test_dense_equivalence(self):
        ...
        loss_ref, loss_scan, fwd_max, max_g = self._run_case(cfg, grad_atol=2e-3, grad_rtol=2e-3)
        print(
